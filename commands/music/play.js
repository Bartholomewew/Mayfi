...
module.exports.execute = async (client, message, args, fromSearch = false) => {
  ...
  if (!voice_channel)
    return client.commands.get("help").execute(client, message, ["play"]);
  ...
  if (!perms.has("CONNECT"))
    return message.reply(
      "⚠️ I do not have permissions to connect to voice channels!"
    );
  if (!perms.has("SPEAK"))
    return message.reply(
      "⚠️ I do not have permissions to speak in voice channels!"
    );

  let videoInfo;

  if (url_regex.test(args[1].toLowerCase())) {
    if (video_regex.test(args[1].toLowerCase())) {
      videoInfo = await youtube.getVideoInfo(args[1], ytdl, message);
      client.queue.handleVideo(videoInfo, message, voice_channel, ytdl);
      return;
    }

    if (playlist_regex.test(args[1].toLowerCase())) {
      ytlist(args[0], ["name", "url"]).then(async result => {
        ...
        for (let i = 0; i < playlistInfo.length; i++) {
          ...
          queue = client.queue.getGuildQueue(message.guild.id);

          if (playlistInfo[i].name == "[Deleted video]") continue;

          try {
            videoInfo = await youtube.getVideoInfo(
              playlistInfo[i].url,
              ytdl,
              message
            );
    ...
    yts(args.join(" "), async (err, result) => {
      if (err) {
        message.reply(
          "⚠️ An error has occurred! Please message my owner about this!"
        );
